				...
				container('jnlp'){
					script{
						scmVars = git branch: '${BRANCH_NAME}',
													...
													url: 'git@github.com:NattaphonMek/sitkmutt-bookinfo-details.git'
					}// End Script
				}// End Container
			}// End Step
		}// End Stage
				...
				container('docker'){
					script{
						docker.withRegistry('https://ghcr.io','registry-bookinfo'){
							docker.build("ghcr.io/nattaphonmek/bookinfo-details:${ENV_NAME}").push()
          ...
          script{
            withKubeConfig([credentialId:'gke-kubeconfig']) {
              withCredentials([file(credentialId:'gke-sa-key-json', variable:'GOOGLE_APPLICATION_CREDENTIALS')]){
                ...
                --namespace bookinfo-${ENV_NAME} bookinfo-${ENV_NAME}-details k8s/helm"
              }// End withCredentials
            }// End withKubeConfig
          }// End script
        }// End container
